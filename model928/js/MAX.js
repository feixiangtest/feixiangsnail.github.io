...
THREE.MAX.prototype = {
    ...
    parse: function(data) {
        ...
        meshobj.script = xmlToJson(xml).SCRIPT;
        ...
        for (var k = 0, l = lum.length; k < l; k++) {
            ...
            for (var i = 0; i < mbtexnum; i++) {
                meshblocks[mbkey]["texture"].push(HexToFloat(lum[k].slice(mbpy + 4 + mbvexlen * 8 * i, mbpy + 4 + mbvexlen * 8 * (i + 1))))
            }
        ...
        group1.indexs=""
           ...
           if(typeof meshobj.script.MATERIAL==="object"&&(!meshobj.script.MATERIAL.length)){
            var geomName = meshobj.script.MATERIAL.attr.name
            
            var diffuMapName = meshobj.script.MATERIAL.DIFFUSE_MAP.MAP["#text"]
            var lightMapName = meshobj.script.MATERIAL.LIGHTING_MAP.MAP["#text"]
            var specularMapName = meshobj.script.MATERIAL.SPECULAR_MAP.MAP["#text"]
            meshobj.script.MATERIAL.GEOM = meshblocks[geomName]
            meshobj.script.MATERIAL.DIFFUSE_MAP[diffuMapName] = modelblob[diffuMapName];
            meshobj.script.MATERIAL.LIGHTING_MAP[lightMapName] = modelblob[lightMapName];
            meshobj.script.MATERIAL[i].SPECULAR_MAP[specularMapName] = modelblob[specularMapName];
            createMesh(meshobj.script.MATERIAL)
            }
            else if(typeof meshobj.script.MATERIAL==="object"&&meshobj.script.MATERIAL.length){
                    for (var i = 0; i < meshobj.script.MATERIAL.length; i++) {
            var geomName = meshobj.script.MATERIAL[i].attr.name
            var diffuMapName = meshobj.script.MATERIAL[i].DIFFUSE_MAP.MAP["#text"]
            var lightMapName = meshobj.script.MATERIAL[i].LIGHTING_MAP.MAP["#text"]
            var specularMapName = meshobj.script.MATERIAL[i].SPECULAR_MAP.MAP["#text"]
            meshobj.script.MATERIAL[i].GEOM = meshblocks[geomName]
            meshobj.script.MATERIAL[i].DIFFUSE_MAP[diffuMapName] = modelblob[diffuMapName];
            meshobj.script.MATERIAL[i].LIGHTING_MAP[lightMapName] = modelblob[lightMapName];
            meshobj.script.MATERIAL[i].SPECULAR_MAP[specularMapName] = modelblob[specularMapName];
            createMesh(meshobj.script.MATERIAL[i])
        }
            }
        ...
        function createMesh(geoMat) {
            ...
            for (var i = 0, l = faces.length; i < l; i++) {
                geometry.faceVertexUvs[0].push([
                    new THREE.Vector2(geoMat.GEOM.texture[0][faces[i].a * 2], geoMat.GEOM.texture[0][faces[i].a * 2 + 1]),
                    new THREE.Vector2(geoMat.GEOM.texture[0][faces[i].b * 2], geoMat.GEOM.texture[0][faces[i].b * 2 + 1]),
                    new THREE.Vector2(geoMat.GEOM.texture[0][faces[i].c * 2], geoMat.GEOM.texture[0][faces[i].c * 2 + 1])
                ]);
            }
            ...
            var material = new THREE.MeshPhongMaterial({
                depthFunc:geoMat.DEPTHFUNC["attr"]["func"],
                specular:new THREE.Color( geoMat.SPECULAR["attr"].r, geoMat.SPECULAR["attr"].g,geoMat.SPECULAR["attr"].b),
                shininess:0
            ...
            if (geoMat.DIFFUSE_MAP[diffuMapName]) {
                ...
                reader.onload = function(e) {
                    ...
                    var texture1 = loader.load(e.target.result)
             ...
             if (geoMat.LIGHTING_MAP[lightMapName]) {
                ...
                reader2.onload = function(e) {
                    ...
                    var texture2 = loader2.load(e.target.result)
                    material.lightMap = texture2;
                    group1.indexs+="b"
                    group1.add(mesh)
                 ...
                 if (geoMat.SPECULAR_MAP[specularMapName]) {
                   
                var reader3 = new FileReader();
                reader3.readAsDataURL(geoMat.SPECULAR_MAP[specularMapName]);
                reader3.onload = function(e) {
                    var loader3 = new THREE.TextureLoader();
                    var texture3 = loader3.load(e.target.result)
