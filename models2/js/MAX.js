...
THREE.MAX.prototype = {
    ...
    parseObj: function(meshobj) {
        ...
        for (var i = 0; i < meshobj.MATERIAL.length; i++) {
            ...
            if (meshobj.MATERIAL[i].DIFFUSE_MAP) {
                var diffuMapName = meshobj.MATERIAL[i].DIFFUSE_MAP.MAP["#text"]
            }
            if (meshobj.MATERIAL[i].LIGHTING_MAP) {
        ...
        function createMesh(geoMat) {
            ...
            if (geoMat.DIFFUSE_MAP && geoMat.DIFFUSE_MAP[diffuMapName]) {
            ...
            if (geoMat.LIGHTING_MAP && geoMat.LIGHTING_MAP[lightMapName]) {
    ...
    parseScript: function(data) {
        ...
        var arrdata = new Uint8Array(data);
          ...
          // var nn=arrdata.subarray(-4);
       
        //var n = tentosixteen(arrdata.subarray(-4));
        var n=tentosixteen(arrdata.subarray(-4));
        ...
        var meshobj = {},
            ...
            meshdata = arrdata.subarray(k[0][0], (k[0][0] + k[0][1])),
            meshscript = arrdata.subarray(k[1][0], (k[1][0] + k[1][2])),
        ...
        function splitMesh() {
            var p = tentosixteen(meshdata.subarray(72, 76));
            var l = tentosixteen(meshdata.subarray(76, 80));
            while (p < meshdata.length) {
                lum.push(pako.inflate(meshdata.subarray((p + 16), (p + 16 + l)), {
                    to: 'ArrayBuffer'
                }))
                p = p + 16 + l;
                if (p < meshdata.length) {
                    l = tentosixteen(meshdata.subarray(p + 8, p + 12))
                }
            }
        }
        splitMesh();
        ...
        for (var k = 0, l = lum.length; k < l; k++) {
            var mbpy;
            var mbnamelen = tentosixteen(lum[k].subarray(0, 4));
            var mbkey = arrToString(lum[k].subarray(4, mbnamelen + 3))
            meshblocks[mbkey] = {}
            mbpy = mbnamelen + 4;
            var mbindicelen = tentosixteen(lum[k].subarray(mbpy, mbpy + 4));
            meshblocks[mbkey]["indices"] = tentosixteensz(lum[k].subarray(mbpy + 4, mbpy + 4 + mbindicelen * 4))
            mbpy = mbpy + 4 + mbindicelen * 4
            var mbvexlen = tentosixteen(lum[k].subarray(mbpy, mbpy + 4))
            meshblocks[mbkey]["vextex"] = HexToFloat(lum[k].subarray(mbpy + 4, mbpy + 4 + mbvexlen * 12))
            mbpy = mbpy + 4 + mbvexlen * 12;
            var mbtexnum = tentosixteen(lum[k].subarray(mbpy, mbpy + 4))
            meshblocks[mbkey]["texture"] = []
            for (var i = 0; i < mbtexnum; i++) {
                var temarr = HexToFloat(lum[k].subarray(mbpy + 4 + mbvexlen * 8 * i, mbpy + 4 + mbvexlen * 8 * (i + 1)))
            ...
            var mbattrnum = tentosixteen(lum[k].subarray(mbpy, mbpy + 4))
            meshblocks[mbkey]["attr"] = HexToFloat(lum[k].subarray(mbpy + 4, mbpy + 4 + mbvexlen * 12))
        }
        var time2 = new Date();
        ...
        function blockInfo(arr) {
            ...
            for (var i = 0; i < n; i++) {
                o[i] = [];
                sli = arr.subarray(-1 * n * 80 - 4)
                a[i] = sli.subarray((i * 80), (i + 1) * 80)
                q[i] = sli.subarray((i * 80 + 16), (i + 1) * 80)
                for (var j = 0; j < 4; j++) {
                    p = a[i].subarray(j * 4, (j + 1) * 4);
                ...
                if (i > 1) {
                    modelblob[filenamearr[i]] = new Blob([arr.subarray(o[i][0], (o[i][0] + o[i][1]))], { 'type': 'image/png' })
                }
            }
